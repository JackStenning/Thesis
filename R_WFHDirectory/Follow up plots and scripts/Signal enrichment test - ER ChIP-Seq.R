...
# Basic sanity checks
if(!all(c("Chr","Start","End") %in% colnames(high_df))) stop("High peaks missing required columns")
...
# create 3 shuffles for High and Low (before expansion), then expand
...
if(length(High_shufs[[1]]) != length(High_peaks_exp)) warning("Some High shuffled peaks were dropped due to chr length constraints; counts may differ.")
...
# 6. Extract signals
# -----------------------------
message("Extracting signals for High peaks (GEO & ENCODE)...")
High_geo_signal   <- extract_mean_signal_from_bigwig(bw_geo, High_peaks_exp)
High_encode_signal<- extract_mean_signal_from_bigwig(bw_encode, High_peaks_exp)

message("Extracting signals for Low peaks (GEO & ENCODE)...")
Low_geo_signal    <- extract_mean_signal_from_bigwig(bw_geo, Low_peaks_exp)
Low_encode_signal <- extract_mean_signal_from_bigwig(bw_encode, Low_peaks_exp)

# shuffled signals (concatenate three shuffles for background distribution)
High_geo_sh_signals <- unlist(lapply(High_shufs, function(sh) extract_mean_signal_from_bigwig(bw_geo, sh)))
Low_geo_sh_signals  <- unlist(lapply(Low_shufs,  function(sh) extract_mean_signal_from_bigwig(bw_geo, sh)))

High_encode_sh_signals <- unlist(lapply(High_shufs, function(sh) extract_mean_signal_from_bigwig(bw_encode, sh)))
Low_encode_sh_signals  <- unlist(lapply(Low_shufs,  function(sh) extract_mean_signal_from_bigwig(bw_encode, sh)))
...
High_wilcox_geo <- wilcox.test(High_geo_signal, High_geo_sh_signals, alternative = "greater")
Low_wilcox_geo  <- wilcox.test(Low_geo_signal,  Low_geo_sh_signals,  alternative = "greater")

High_wilcox_encode <- wilcox.test(High_encode_signal, High_encode_sh_signals, alternative = "greater")
Low_wilcox_encode  <- wilcox.test(Low_encode_signal,  Low_encode_sh_signals,  alternative = "greater")

# empirical p-values
High_emp_geo <- empirical_pval(High_geo_signal, High_geo_sh_signals)
Low_emp_geo  <- empirical_pval(Low_geo_signal,  Low_geo_sh_signals)

High_emp_encode <- empirical_pval(High_encode_signal, High_encode_sh_signals)
Low_emp_encode  <- empirical_pval(Low_encode_signal,  Low_encode_sh_signals)

# report
message("---- Results (GEO bigWig) ----")
message("High peaks: Wilcoxon p = ", signif(High_wilcox_geo$p.value, 4),
        "; empirical p (vs 3 shuffles combined) = ", signif(High_emp_geo$p_emp, 4),
        "; obs mean = ", signif(High_emp_geo$obs_mean,4))
message("Low peaks:  Wilcoxon p = ", signif(Low_wilcox_geo$p.value, 4),
        "; empirical p (vs 3 shuffles combined) = ", signif(Low_emp_geo$p_emp, 4),
        "; obs mean = ", signif(Low_emp_geo$obs_mean,4))

message("---- Results (ENCODE bigWig) ----")
message("High peaks: Wilcoxon p = ", signif(High_wilcox_encode$p.value, 4),
        "; empirical p = ", signif(High_emp_encode$p_emp, 4),
        "; obs mean = ", signif(High_emp_encode$obs_mean,4))
message("Low peaks:  Wilcoxon p = ", signif(Low_wilcox_encode$p.value, 4),
        "; empirical p = ", signif(Low_emp_encode$p_emp, 4),
        "; obs mean = ", signif(Low_emp_encode$obs_mean,4))
...
# -------------------------------
plot_df <- bind_rows(
  data.frame(type="High_obs_GEO",       value=High_geo_signal),
  data.frame(type="High_shuf_GEO",      value=High_geo_sh_signals),
  data.frame(type="Low_obs_GEO",        value=Low_geo_signal),
  data.frame(type="Low_shuf_GEO",       value=Low_geo_sh_signals),
  data.frame(type="High_obs_ENCODE",    value=High_encode_signal),
  data.frame(type="High_shuf_ENCODE",   value=High_encode_sh_signals),
  data.frame(type="Low_obs_ENCODE",     value=Low_encode_signal),
  data.frame(type="Low_shuf_ENCODE",    value=Low_encode_sh_signals)
)
...
# -------------------------------
obs_vs_shuf <- list(
  c("High_obs_GEO", "High_shuf_GEO"),
  c("Low_obs_GEO",  "Low_shuf_GEO"),
  c("High_obs_ENCODE", "High_shuf_ENCODE"),
  c("Low_obs_ENCODE",  "Low_shuf_ENCODE")
)

high_vs_low <- list(
  c("High_obs_GEO", "Low_obs_GEO"),
  c("High_obs_ENCODE", "Low_obs_ENCODE")
)

my_comparisons <- c(obs_vs_shuf, high_vs_low)

###############################################################################
# Simple function to save plots only
###############################################################################
save_plots_tiff <- function(outdir = "CC_enrichment_plots_tiff", plots = list()){
...
# Only compare High vs Low within observed
