...
tiff("pyCalling Cards Peaks and ER ChIP-Seq Overlap.tiff", 
...
pyCC_Figure <- ggplot(data = pyCC_Finalpercentage_table, 
  ...
  theme_minimal(base_size = 14) +  # increase all text by 2 points (default is 12)
  geom_text(aes(label = round(PercentageOverlapping, 1)), 
            vjust = -0.5, 
            size = 5) +  # keeps bar labels at current size
  coord_cartesian(expand = FALSE, clip = "off") +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.y = element_text(size = 15))  # axis title slightly bigger
