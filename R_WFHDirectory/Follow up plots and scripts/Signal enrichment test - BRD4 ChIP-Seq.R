...
# 6. Extract signals
# -----------------------------
message("Extracting signals for High peaks (Liu & Zheng)...")
High_Liu_signal   <- extract_mean_signal_from_bigwig(bw_liu, High_peaks_exp)
High_Zheng_signal<- extract_mean_signal_from_bigwig(bw_zheng, High_peaks_exp)
...
High_Liu_sh_signals <- unlist(lapply(High_shufs, function(sh) extract_mean_signal_from_bigwig(bw_liu, sh)))
Low_Liu_sh_signals  <- unlist(lapply(Low_shufs,  function(sh) extract_mean_signal_from_bigwig(bw_liu, sh)))

High_Zheng_sh_signals <- unlist(lapply(High_shufs, function(sh) extract_mean_signal_from_bigwig(bw_zheng, sh)))
...
High_wilcox_Liu <- wilcox.test(High_Liu_signal, High_Liu_sh_signals, alternative = "greater")
Low_wilcox_Liu  <- wilcox.test(Low_Liu_signal,  Low_Liu_sh_signals,  alternative = "greater")

High_wilcox_Zheng <- wilcox.test(High_Zheng_signal, High_Zheng_sh_signals, alternative = "greater")
Low_wilcox_Zheng  <- wilcox.test(Low_Zheng_signal,  Low_Zheng_sh_signals,  alternative = "greater")

# empirical p-values
High_emp_Liu <- empirical_pval(High_Liu_signal, High_Liu_sh_signals)
Low_emp_Liu  <- empirical_pval(Low_Liu_signal,  Low_Liu_sh_signals)

High_emp_Zheng <- empirical_pval(High_Zheng_signal, High_Zheng_sh_signals)
Low_emp_Zheng  <- empirical_pval(Low_Zheng_signal,  Low_Zheng_sh_signals)

# report
message("---- Results (Liu bigWig) ----")
message("High peaks: Wilcoxon p = ", signif(High_wilcox_Liu$p.value, 4),
        "; empirical p (vs 3 shuffles combined) = ", signif(High_emp_Liu$p_emp, 4),
        "; obs mean = ", signif(High_emp_Liu$obs_mean,4))
...
message("High peaks: Wilcoxon p = ", signif(High_wilcox_Zheng$p.value, 4),
        "; empirical p = ", signif(High_emp_Zheng$p_emp, 4),
        "; obs mean = ", signif(High_emp_Zheng$obs_mean,4))
...
# -------------------------------
plot_df <- bind_rows(
  data.frame(type="High_obs_Liu",       value=High_Liu_signal),
  data.frame(type="High_shuf_Liu",      value=High_Liu_sh_signals),
  data.frame(type="Low_obs_Liu",        value=Low_Liu_signal),
  data.frame(type="Low_shuf_Liu",       value=Low_Liu_sh_signals),
  data.frame(type="High_obs_Zheng",    value=High_Zheng_signal),
  data.frame(type="High_shuf_Zheng",   value=High_Zheng_sh_signals),
...
# -------------------------------
obs_vs_shuf <- list(
  c("High_obs_Liu", "High_shuf_Liu"),
  c("Low_obs_Liu",  "Low_shuf_Liu"),
  c("High_obs_Zheng", "High_shuf_Zheng"),
  c("Low_obs_Zheng",  "Low_shuf_Zheng")
)

high_vs_low <- list(
  c("High_obs_Liu", "Low_obs_Liu"),
  c("High_obs_Zheng", "Low_obs_Zheng")
)
