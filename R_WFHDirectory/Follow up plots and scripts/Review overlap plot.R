...
#Create table to plot
pyCC_Finalpercentage_table = data.frame(Name=c('Peakset_2_GEO', 'Peakset_2_Encode'),
                                       Total=rep(c(ESR1_pyCC_2_total, ESR1_pyCC_2_total)),
                                       Overlap_withChIP= c(ESR1_pyCC_2_GEO_q_overlap, ESR1_pyCC_2_Encode2_q_overlap),
                                       PercentageOverlapping= c(Percentage_ESR1_pyCC_2_GEO_q, Percentage_ESR1_pyCC_2_Encode2_q))

#Then we plot
graphorder= c('Peakset_2_GEO', 'Peakset_2_Encode')
                         ...
                         stat("identity"))) +  geom_col(color = "black",
                                                        show.legend = FALSE) +  ylim(0, 100) + labs(x = "Name of CC peak file and ChIP Dataset", 
                                                                                                    ...
                                                                                                    subtitle = "Determining percentage of overlapping peaks from pyCalling Card ESR1 peak files with either GSE109820 or \nENCFF063JMY ChIP-Seq datasets within 1 kbp") +  theme_dark() + geom_text(aes(label = round(PercentageOverlapping, 1)), # Add percentage as text
                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                             size = 4) + coord_cartesian(expand = FALSE, clip = "off")
