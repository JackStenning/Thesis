...
files <- list.files(
  input_dir,
  pattern = "^basic_Chia_ChiapetRep[123]_peak_data_ER_WT2",
  full.names = TRUE
)
...
#Colnames
colnames(all_data)[1:22] <- c("chr_chia","start_chia","end_chia","name","score","strand",
                              "thickStart","thickEnd","itemRgb",
                              "blockCount","blockSizes","blockStarts", "Chr_ChIP",	"Start_ChIP",	"End_ChIP",	"Name",	"Score",	"Strand",	"Signal Value",	"pvalue",	"qvalue",	"Peak")

# Subset relevant columns
subset_data <- all_data %>%
  select(chr_chia, start_chia, end_chia, name,
         Chr_ChIP, Start_ChIP, End_ChIP, Replicate)
...
anchor1 <- subset_data %>%
  ...
  rename(chr_chia_anchor1 = chr_chia,
         ...
         Chr_ChIP_anchor1 = Chr_ChIP,
         Start_ChIP_anchor1 = Start_ChIP,
         End_ChIP_anchor1 = End_ChIP)
...
anchor2 <- subset_data %>%
  ...
  rename(chr_chia_anchor2 = chr_chia,
         ...
         Chr_ChIP_anchor2 = Chr_ChIP,
         Start_ChIP_anchor2 = Start_ChIP,
         End_ChIP_anchor2 = End_ChIP)
...
# Plot
tiff("Percentage of ChIA-PET Interactions Overlapping with high stringency Calling Card Peaks.tiff", width = 3510, height = 2481, res = 300)
ggplot(plot_df, aes(x = "", y = Percentage, fill = Status)) +
  geom_bar(stat = "identity", width = 0.5) +
  coord_flip() +
  geom_text(aes(label = paste0(round(Percentage, 1), "%")), 
            position = position_stack(vjust = 0.5), size = 5) +
  labs(title = "Percentage of ChIA-PET Interactions Overlapping with high stringency Calling Card Peaks",
       x = NULL, y = "Percentage") +
  theme_minimal() +
