...
# Read the CSV file
df <- read_csv("trimmedER Intensity from core_NoAmilorideCompleteSphere.csv")
...
for (metric in metrics) {
  for (day in days) {
    ...
    ggsave(filename = paste0("ER_", metric, "_Day", day, ".tiff"),
           plot = p,
           width = plot_width,
           height = plot_height,
           dpi = plot_dpi)
...
for (metric in metrics) {
  p <- ggplot(df_long %>% filter(Metric == metric),
    ...
    geom_line(alpha = 0.7) +
    facet_wrap(~Day) +
    labs(title = paste("ER -", metric, "across Days 7, 9, 11, 13"),
         x = "Bin (Distance from Center)",
         y = metric,
         color = "Day") +
    theme_minimal() +
  ...
  ggsave(filename = paste0("ER_", metric, "_Faceted.tiff"),
         plot = p,
