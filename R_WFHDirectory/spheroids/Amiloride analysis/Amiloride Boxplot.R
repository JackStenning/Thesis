...
write_xlsx(tukey_sheets, "early_tukey_anova_results_by_day.xlsx")
...
#Day 7 - 13
summary_data_filt <- long_data_filtered %>%
  group_by(Day, Condition) %>%
  summarise(
    mean_size = mean(Size, na.rm = TRUE),
    sem = sd(Size, na.rm = TRUE) / sqrt(n()),
...
# Line plot with error bars
ggplot(summary_data_filt, aes(x = Day, y = mean_size, color = Condition, group = Condition)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_size - sem, ymax = mean_size + sem), width = 0.3) +
  labs(
...
# Export to Excel file
write_xlsx(Ltukey_sheets, "late_tukey_anova_results_by_day.xlsx")

#Line plot
# Summary stats by Day and Condition
summary_data_L <- long_data_L %>%
  group_by(Day, Condition) %>%
  summarise(
    mean_size = mean(Size, na.rm = TRUE),
    sem = sd(Size, na.rm = TRUE) / sqrt(n()),
...
# Line plot with error bars
ggplot(summary_data_L, aes(x = Day, y = mean_size, color = Condition, group = Condition)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_size - sem, ymax = mean_size + sem), width = 0.3) +
  labs(
...
#Day 7 - 13
summary_data_filt_L <- long_data_L_filtered %>%
  group_by(Day, Condition) %>%
  summarise(
    mean_size = mean(Size, na.rm = TRUE),
    sem = sd(Size, na.rm = TRUE) / sqrt(n()),
...
# Line plot with error bars
ggplot(summary_data_filt_L, aes(x = Day, y = mean_size, color = Condition, group = Condition)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_size - sem, ymax = mean_size + sem), width = 0.3) +
  labs(
